...
export default function CaseStudiesPage() {
  return (
    <main className="max-w-4xl mx-auto py-12 sm:py-16 px-4 sm:px-6">
      <h1 className="text-3xl sm:text-4xl font-bold mb-8 text-center">Case Studies</h1>
      <div className="space-y-6 sm:space-y-8">
        {caseStudies.map((cs) => (
          <div key={cs.title} className="bg-white rounded-lg shadow p-6 hover:shadow-lg transition">
            <h2 className="text-lg sm:text-2xl font-semibold mb-2">{cs.title}</h2>
            <p className="text-gray-600 text-sm sm:text-base">{cs.summary}</p>
          </div>
        ))}
      </div>
    </main>
